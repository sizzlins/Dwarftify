                    ...
                    subviews = {
                        widgets.FilteredList{
                            ...
                            on_submit = function(idx, choice) 
                                local track = table.remove(STATE.queue, idx)
                                table.insert(STATE.queue, 1, track)
                                saveConfig()
                                playTrackNow(track)
                            end,
...
function Dwarftify:getActiveList()
    if STATE.active_tab == 1 then
        if self.subviews.list_browse_tracks.has_focus then return self.subviews.list_browse_tracks end
        return nil
...
function Dwarftify:updateFilters()
    ...
    local function makeTrackChoice(track, is_queue)
        ...
        return {
            text = {
                {text = heart .. ' ', pen = COLOR_LIGHTRED},
                {text = track.title, pen = COLOR_WHITE},
                {text = ' - ', pen = COLOR_GREY},
                {text = track.author, pen = COLOR_CYAN}
            },
            track = track,
            search_key = track.title:lower() .. " " .. track.author:lower()
        }
    ...
    if STATE.active_tab == 1 then
        ...
        if STATE.search_string ~= "" then
            self.subviews.browse_tracks_panel.frame = {t = 0, l = 0, r = 0, b = 0}
            self.subviews.browse_tracks_panel.frame_title = 'Global Search Results'
            ...
            self.subviews.browse_tracks_panel.frame = {t = 0, l = 57, r = 0, b = 0}
            local author_data = self.authors[self.selected_author_idx]
            self.subviews.browse_tracks_panel.frame_title = author_data and (author_data.name .. "'s Tracks") or 'Tracks'
            if author_data then
    ...
    if STATE.active_tab == 2 then
        ...
        for i, track in ipairs(STATE.queue) do
            ...
            if search_lower == "" or choice.search_key:find(search_lower, 1, true) then
                choice.text = string.format("%02d. %s", i, choice.text)
...
function Dwarftify:onInput(keys)
    if keys.LEAVESCREEN then
        if STATE.active_tab == 1 and STATE.search_string == "" and self.subviews.list_browse_tracks.has_focus then
            self.subviews.list_artists:setFocus(true)
            return true
        end
    elseif keys._STRING == 9 then -- TAB
